...
/*import com.summitprojects.core3.externalAssets.Path;
...
public class HairParticle extends EventDispatcher {
	...
	**/
	public function HairParticle(circle:Sprite, verbosity:int=0) {
		_circle = circle;
		init(verbosity);
	...
	// INITIALIZATION ============================================================================================================
	private function init(verbosity:Number):void {
		createVars(verbosity);
		createChildren();
		initButtons();
		initEvents();
	...
	private function createVars(verbosity:Number):void {
	...
	**/
	public function draw(canvas:BitmapData) : void {
		...
		if (_active) {
	    	...
	    	//Draw Routine here
	    	// ofSetColor(_color, _color, _color, _alpha); //set color and alpha blend
	    	// ofCircle(_x, _y, _scale); //draw a circle
			// var argb:Number = (_alpha << 24) | (_color << 16) | (_color << 8) | (_color);
			// var argb:Number = (_color << 16) | (_color << 8) | (_color);
			// trace("argb.toString(24) :: " + argb + " :: " + argb.toString(24));

			// var rect:Rectangle = new Rectangle(_x, _y, _scale, _scale);
			// canvas.fillRect( rect, _color );
			
			
			// runs slowly
			// drawCircle( canvas, _x, _y, _scale, argb);
