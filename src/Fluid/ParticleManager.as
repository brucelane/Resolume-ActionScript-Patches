		...
		public function update(bmp:BitmapData, lines:Boolean):void
		{
			...
			while (p)
			{
				if (p.alpha > 0) 
				{
					...
					if(FluidSolva.drawFluid){
						c = a << 24 | a << 16 | a << 8 | a;
					} else {
						vxNorm = p.vx * FluidSolva.isw;
						vyNorm = p.vy * FluidSolva.ish;
					...
					if (lines) {
						//drawLine(bmp, int(p.px - FluidSolva.mx + .5), int(p.py - FluidSolva.my + .5), int(p.x + .5), int(p.y + .5), c);
						drawLine(bmp, int(p.px + .5), int(p.py + .5), int(p.x + .5), int(p.y + .5), c);
					} else {
